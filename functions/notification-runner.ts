...
const shouldNotify = async (courseId: number, userId: string) => {
  const alreadyNotified = await dynamodb
    .getItem({
      AttributesToGet: ["NotifiedAt"],
...
const notifyUserOfCourses = async (
  userId: string,
  ...
  try {
    ...
    console.log(
      `Notification for user ${userId} sent, subscription ${subscriptionId}`,
    );
    ...
    console.error(
      `Notification email for user ${userId} subscription ${subscriptionId} failed: ${err.message}`,
    );
  }
};
...
const asyncFilter = async (
  arr: any[],
  predicate: (course: any) => Promise<boolean>,
) =>
...
export const runNotifications: Handler = async (event) => {
  ...
  const allRequests = await scanTable(
    dynamodb,
...
const doHandleSubscriptionNotification = async (
  userId: string,
  subscriptionId: string,
  tokens: string[],
) => {
...
export const handleSqsHandleNotificationEvent: Handler = async (event) => {
  for (const record of event.Records) {
    ...
    console.log(
      `Handling message ${record.messageId} from the queue. User ${message.userId} Subscription ${message.subscriptionId}`,
    );
    await doHandleSubscriptionNotification(
      message.userId,
...
export const getMyNotifiedCourses: Handler = async (event) => {
  ...
  const notifications = await queryTable(
    dynamodb,
