...
export const populateCourseIndex = async (force?: boolean) => {
  ...
  if (!force && lastUpdatedInDb && lastUpdatedInDb === currentIndexVersion) {
    console.log(`Course index is at version ${lastUpdatedInDb}, no need to refetch.`);
    return;
  }
  ...
  for (const doc of allData) {
    courseIndex.add(doc.CourseId.N, doc.Name?.S + ' ' + doc.Location?.S + ' ' + doc.IntroText?.S);
    courseMap.set(doc.CourseId.N, doc);
  }
  
  console.log(`Course index updated ${allData.length} courses, last update ${lastUpdatedInDb}.`);
}
...
export const checkCourseIndexUpdated = async(): Promise<number | undefined> => {
...
export const findCourses = async (tokens: string[]) => {
    ...
    console.log(`findCourses: Tokens ${JSON.stringify(tokens)} -> keywords ${JSON.stringify(keywords)} / not-keywords ${JSON.stringify(notKeywords)}`)
