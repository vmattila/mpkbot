...
const crawlCourseListing = async (unitId, startDate, lastCourseId) => {
  ...
  if (responseData.length == 100) {
    ...
    if (loopLastCourseId === lastCourseId) {
      console.log(
        `Got same results back from ${url}, so adding one day to the loop.`,
      );
...
const updateCourseFromJson = async (courseId: number, courseJsonItem: any) => {
  ...
  const found = await dynamodb
    .getItem({
      AttributesToGet: ["LastCrawledAt", "JsonHash", "CrawlPending"],
  ...
  if (found && found.Item) {
    ...
    if (found.Item.CrawlPending?.BOOL) {
      console.log(
        `Course ${courseId} ${courseName}, crawl is already pending.`,
      );
    } else if (found.Item.JsonHash?.S !== jsonHash) {
      console.log(
        `Course ${courseId} ${courseName} hash has changed. Feeding to re-crawl.`,
      );
      ...
      if (moment().diff(lastUpd, "hours") > 24) {
        ...
        console.log(
          `Course ${courseId} ${courseName} is crawled within 24 hours, not crawling again.`,
        );
      }
    } else {
      console.log(
        `Course ${courseId} ${courseName} is still valid, not re-crawling.`,
      );
    }
  } else {
    await pushToCrawl(courseId, courseJsonItem);
...
const doCourseCrawl = async (courseId: number, courseJsonItem: any) => {
  ...
  const response = await axios.get(
    `https://koulutuskalenteri.mpk.fi/Default.aspx?tabid=1054&id=${courseId}`,
  );
  const responseData = response.data;

  const $ = cheerio.load(responseData);
  const courseName = courseJsonItem.Nimi;
  const ajankohta = $('#product-details table th:contains("Ajankohta") ~ td')
    .text()
    .trim();

  // Koitetaan tulkita loppuaika annetusta formaatista
  const loppuaikaMoment = moment(
    courseJsonItem.LoppuaikaStr,
    "D.M.YYYY H.mm",
    true,
  );
  let loppuaika;
  ...
  const paikka = $('#product-details table th:contains("Paikkakunta") ~ td')
    .text()
    .trim();

  const tavoite = $('#tabs > #tabs-1 h2:contains("Tavoite") + p').text().trim();
  const soveltuvuus = $(
    '#tabs > #tabs-1 h2:contains("Kenelle kurssi soveltuu") + p',
  )
    .text()
    .trim();
  const esitiedot = $('#tabs > #tabs-1 h2:contains("Esitiedot") + p')
    .text()
    .trim();
  const sis = $('#tabs > #tabs-1 h2:contains("Sisältö") + p').text().trim();
  const hinta = $('#tabs > #tabs-1 h2:contains("Hinta") + p').text().trim();
  const kopa = $('#tabs > #tabs-1 h2:contains("Koulutuspaikka") + p')
    .text()
    .trim();
  const yhteystiedot = $('#tabs > #tabs-1 h2:contains("Yhteystiedot") + p')
    .text()
    .trim();

  const teksti = [tavoite, soveltuvuus, esitiedot, sis].join("\n\n");

  const ttlTime = loppuaika
    ? loppuaika + 3600 * 24
    : courseJsonItem.Alkuaika + 3600 * 24;
  ...
  await dynamodb
    ...
    .promise();
  console.log(
    `Course ${courseId} ${courseName} successfully ingested to DynamoDB`,
  );
...
export const crawl: Handler = async (event) => {
  ...
  const units = [
    1, 2, 3, 5, 10, 8, 7, 22, 15, 11, 12, 21, 4, 6, 9, 18, 20, 19, 17, 16,
  ];
...
export const handleSqsCrawlEvent: Handler = async (event) => {
  for (const record of event.Records) {
    ...
    console.log(
      `Handling message ${record.messageId} from the queue. Course ${message.courseId} ${message.courseJsonItem}`,
    );
